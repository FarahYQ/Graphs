...
var cloneGraph = function(graph) {
    ...
    while (queue.length >0) {
        ...
        let graphCopy = new UndirectedGraphNode(current.label);
        for (let i=0; i<current.neighbors.length; i++) {
            if (!visited.includes(current.neighbors[i])) {
                let neighborCopy = new UndirectedGraphNode(current.neigbors[i]);
                queue.push(neighborCopy);
                visited.push(current.neighbors[i]);
            } else {
                let neighborCopy = visited.filter(el => el.label === current.neighbors[i].label)
            }
            graphCopy.neighbors.push(neighborCopy)
        } 
    }
    return visited[0];
};